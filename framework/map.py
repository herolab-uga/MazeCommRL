...
class Map:
	...
	def getDistance(self, x, y):
		return math.dist([x, self.width], [y, self.height])

	def getReward(self, x , y):
		cell = getCell(x , y)
		d_curr = getDistance(cell.x,cell.y)
		d_prev = getDistance(cell.x-1,cell.y-1)
		return sum((1/d_curr - 1/d_prev) + (1 - 1/d_curr) for i in range(2))
